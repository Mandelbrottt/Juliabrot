Shader "Juliabrot/Mandelbrot"
{
	...
	SubShader 
	{
		...
		Pass 
		{
			...
			int Compute(double2 c, int threshold, int order) {
				double2 ab = 0;
			...
			fixed4 FragmentFunc(v2f IN) : SV_Target {
				...
				double2 coord = position + bounds * double2(IN.uv);

				int numIters = _NumIterations;

				int iter = Compute(coord, numIters, _Order);
...
// void multiplyByteInInt(
// 	in    int indexA, 
// 	in    int arrayA[NUM1_SZ], 
// 	in    int indexB, 
// 	in    int arrayB[NUM2_SZ], 
// 	inout int arrayC[NUM1_SZ + NUM2_SZ]
// ) {
// 	int subIndexA = indexA % 4;
// 	int adjIndexA = indexA / 4;

// 	int byteA = arrayA[adjIndexA];
// 	byteA = byteA >> (subIndexA * 8);
// 	byteA = byteA & 0xFF;

// 	int subIndexB = indexB % 4;
// 	int adjIndexB = indexB / 4;
	
// 	int byteB = arrayB[adjIndexA];
// 	byteB = byteB >> (subIndexB * 8);
// 	byteB = byteB & 0xFF;

// 	int byteC = bytaA + byteB;
// 	int carry = byteC - (byteC % 10)
// 	byteC = byteC % 10;

// 	int indexC      = indexA + indexB;
// 	int indexCarryC = indexC - 1;

// 	int subIndexC = indexC % 4;
// 	int adjIndexC = indexC / 4;

// 	int byteC = arrayC[adjIndexC];
// 	byteC = byteC >> (subIndexC)

// 	int subIndexCarryC = indexCarryC % 4;
// 	int adjIndexCarryC = indexCarryC / 4;
// }
